...
describe('<Signup />', () => {
  ...
  it('renders the sign up page with a form submission and signs a user up', async () => {
    ...
    await act(async () => {
      ...
      await fireEvent.change(getByPlaceholderText('Username'), { target: { value: 'jose' } });
      await fireEvent.change(getByPlaceholderText('Full name'), {
        target: { value: 'Jose Nieves-Diaz' }
      });
      ...
      expect(document.title).toEqual('Sign Up - Tafoo');
      await expect(doesUsernameExist).toHaveBeenCalled();
      await expect(doesUsernameExist).toHaveBeenCalledWith('jose');

      await waitFor(() => {
        expect(mockHistoryPush).toHaveBeenCalledWith(ROUTES.DASHBOARD);
        expect(getByPlaceholderText('Username').value).toBe('jose');
        expect(getByPlaceholderText('Full name').value).toBe('Jose Nieves-Diaz');
